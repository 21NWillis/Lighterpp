...
__global__ void aggregation_kernel_multihead(float* out, const __half* value_cache, const float* att_scores, int n_heads, int seq_len, int head_size, int gqa_factor, int att_stride) {
    ...
    const __half* v_cache_head = value_cache + kv_head * (att_stride * head_size);
    float* out_head = out + head * head_size;
    
    float sum = 0.0f;
    for (int t = 0; t < seq_len; t++) {
        sum += __half2float(v_cache_head[t * head_size + i]) * att_head[t];
    }
